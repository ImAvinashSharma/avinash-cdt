...
- The idea hear is that we will be creating a s3 bucket and performing a check whether the bucket already exists or not (any ways we are creating the bucket name dynamically which means a alpha numeric sting is generated to make sure the bucket name is unique).
...
- Then we create a internet gateway that enables instances as it allows a VPC to connect to the Internet.
