...
- The idea hear is that we will be creating a s3 bucket and performing a check whether the bucket already exists or not (any ways we are creating the bucket name dynamically means a alpha numeric stings is generated to make sure the bucket name is unique).
